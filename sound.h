...
namespace sound {
    ...
    inline short GenerateWave(WaveType type, double time, double duration, double frequency, double amp, ADSR env) {
        double envelope = ApplyADSR(time, duration, env);
        double cycles = time * frequency;
        double rad = TWOPI * cycles;
        short amplitude = static_cast<short>(32767 * amp * envelope);
